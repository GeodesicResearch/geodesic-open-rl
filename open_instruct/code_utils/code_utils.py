...
def _submit_to_pool(fn, *args, timeout: float) -> tuple[list[int], list[float], dict] | None:
...
def _pool_run_tests(program: str, tests: list[str], total_timeout: int) -> tuple[list[int], list[float], dict]:
    """Pool worker: run assert-based tests (container provides OS-level isolation).

    Returns (results, runtimes, hacks) where hacks tracks detected hack strategies.
    """
    n = len(tests)
    results = [0] * n
    runtimes = [-1.0] * n
    ...
    try:
        ...
        for idx, test in enumerate(tests):
            try:
                ...
                exec(test, execution_context)
                end_time = time.time()
                # Check __eq__ canary for observability (flag but don't zero results)
                if _is_eq_hack(test, execution_context):
                    hacks["eq_hack"][idx] = 1
                results[idx] = 1
                runtimes[idx] = end_time - start_time
...
def get_successful_tests_hackable(
    ...
    return result if result is not None else ([0] * len(tests), [-1.0] * len(tests), _default_hacks(test_ct))
